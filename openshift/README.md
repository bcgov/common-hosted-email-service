# NodeMailer Proof of Concept on Openshift
...
```sh
oc create -n jcyvmi-<env> configmap nmpoc-server-config \
...
```sh
oc -n jcyvmi-<env> process -f openshift/app.bc.yaml -p REPO_NAME=nodemailer-poc
 -p JOB_NAME=master -p SOURCE_REPO_URL=https://github.com/jujaga/nodemailer-poc.git -p SOURCE_REPO_REF=master -o yaml | oc -n jcyvmi-<env> create -f -
```
...
```sh
oc -n jcyvmi-<env> start-build <buildname> --follow
```
...
```sh
oc -n jcyvmi-<env> tag <buildname>:latest <buildname>:master
```
...
```sh
oc -n jcyvmi-<env> process -f openshift/app.dc.yaml -p REPO_NAME=nodemailer-poc -p JOB_NAME=master -p NAMESPACE=jcyvmi-<env> -p APP_NAME=nmpoc -p HOST_ROUTE=nmpoc-master-jcyvmi-<env>.pathfinder.gov.bc.ca -o yaml | oc -n jcyvmi-<env> apply -f -
```
...
```sh
oc -n jcyvmi-<env> rollout latest dc/<buildname>-master
```
...
```sh
oc delete all -n jcyvmi-dev --selector app=nmpoc-pr-<PRNUMBER>
```
...
```sh
oc new-build -n jcyvmi-tools registry.access.redhat.com/rhscl/nodejs-8-rhel7:latest~https://github.com/bcgov/nr-get-token.git#master --context-dir=frontend --name=get-token-frontend --dry-run -o yaml > openshift/frontend.bc.yaml
...
```sh
oc process -n jcyvmi-tools -f openshift/frontend.bc.yaml -o yaml | oc create -n jcyvmi-tools -f -
```
...
```sh
oc new-build -n jcyvmi-tools --docker-image=docker-registry.default.svc:5000/bcgov/s2i-caddy:v1-stable --source-image=frontend:latest --source-image-path=/opt/app-root/src/dist:tmp -D $'FROM docker-registry.default.svc:5000/bcgov/s2i-caddy:v1-stable\nCOPY tmp/dist/ /var/www/html/\nCMD /tmp/scripts/run' --dry-run --name=get-token-frontend-static -o yaml > openshift/frontend-static.bc.yaml
...
```sh
oc process -n jcyvmi-tools -f openshift/frontend-static.bc.yaml -o yaml | oc create -n jcyvmi-tools -f -
```

### Tag the latest build and migrate it to the correct project namespace

```sh
oc tag -n jcyvmi-dev jcyvmi-tools/frontend-static:latest frontend-static:dev --reference-policy=local
```
...
```sh
oc new-app -n jcyvmi-dev --image-stream=frontend-static:dev --name=get-token-frontend --dry-run -o yaml > openshift/frontend-static.dc.yaml
```

### Process and Apply the Application Deployment

```sh
oc process -n jcyvmi-dev -f openshift/frontend-static.dc.yaml -o yaml | oc create -n jcyvmi-dev -f -
oc create -n jcyvmi-dev route edge frontend --service=frontend --port=2015-tcp
```
