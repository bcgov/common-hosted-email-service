...
oc create -n b160aa-<env> configmap ches-keycloak-config \
...
```sh
oc create -n b160aa-<env> secret generic ches-keycloak-secret \
...
```sh
oc -n b160aa-<env> process -f openshift/app.bc.yaml -p REPO_NAME=common-hosted-email-service
 -p JOB_NAME=master -p SOURCE_REPO_URL=https://github.com/bcgov/common-hosted-email-service.git -p SOURCE_REPO_REF=master -o yaml | oc -n b160aa-<env> create -f -
```
...
```sh
oc -n b160aa-<env> start-build <buildname> --follow
```
...
```sh
oc -n b160aa-<env> tag <buildname>:latest <buildname>:master
```
...
```sh
oc -n b160aa-<env> process -f openshift/app.dc.yaml -p REPO_NAME=common-hosted-email-service -p JOB_NAME=master -p NAMESPACE=b160aa-<env> -p APP_NAME=ches -p HOST_ROUTE=ches-<env>.apps.silver.devops.gov.bc.ca -o yaml | oc -n b160aa-<env> apply -f -
```
...
```sh
oc -n b160aa-<env> rollout latest dc/<buildname>-master
```
...
```sh
oc delete all,secret,pvc,networkpolicy,rolebinding -n b160aa-dev -l app=ches-pr-<PRNUMBER>
oc delete all,svc,cm,sa,role,secret -n b160aa-dev -l cluster-name=pr-<PRNUMBER>
oc delete all -n b160aa-dev -l app=redis-pr-<PRNUMBER>
```
...
```sh
oc new-build -n b160aa-tools registry.access.redhat.com/rhscl/nodejs-8-rhel7:latest~https://github.com/bcgov/nr-get-token.git#master --context-dir=frontend --name=get-token-frontend --dry-run -o yaml > openshift/frontend.bc.yaml
...
```sh
oc process -n b160aa-tools -f openshift/frontend.bc.yaml -o yaml | oc create -n b160aa-tools -f -
```
...
```sh
oc new-build -n b160aa-tools --docker-image=docker-registry.default.svc:5000/bcgov/s2i-caddy:v1-stable --source-image=frontend:latest --source-image-path=/opt/app-root/src/dist:tmp -D $'FROM docker-registry.default.svc:5000/bcgov/s2i-caddy:v1-stable\nCOPY tmp/dist/ /var/www/html/\nCMD /tmp/scripts/run' --dry-run --name=get-token-frontend-static -o yaml > openshift/frontend-static.bc.yaml
...
```sh
oc process -n b160aa-tools -f openshift/frontend-static.bc.yaml -o yaml | oc create -n b160aa-tools -f -
```

### Tag the latest build and migrate it to the correct project namespace

```sh
oc tag -n b160aa-dev b160aa-tools/frontend-static:latest frontend-static:dev --reference-policy=local
```
...
```sh
oc new-app -n b160aa-dev --image-stream=frontend-static:dev --name=get-token-frontend --dry-run -o yaml > openshift/frontend-static.dc.yaml
```

### Process and Apply the Application Deployment

```sh
oc process -n b160aa-dev -f openshift/frontend-static.dc.yaml -o yaml | oc create -n b160aa-dev -f -
oc create -n b160aa-dev route edge frontend --service=frontend --port=2015-tcp
```
